...
*** (2) omit 
	...
	replace 		male_ospec = 1 if sex_salesmanager1 == 0 
 	...
 	save 			"$fil\regression-ready\household-total_y2-t.dta", replace	
...
* adjusting for inflation 	
	...
	*** adding housing and utilities with furnishings 
	...
	generate 		totalexp = rexpagg / 1.661
 	...
 	save 			"$fil\regression-ready\household-total_y2_t.dta", replace	
...
* append all 
	...
	merge 			m:1 case_id ea_id region district HHID using "$fil\regression-ready\household-total_y2-t.dta"	
	drop 			_merge 
...
* ********************************************************************** 
	...
	append 			using "$fil\regression-ready\householdstack09.dta"
	save 			 "$fil\regression-ready\householdstack.dta", replace	

* sales values by manager 
	bys case_id HHID: gen lnvaluejoint_jspec = asinh(valuejoint_jspec)
	bys case_id HHID: gen lnvaluefemale_jspec = asinh(valuefemale_jspec)
	bys case_id HHID: gen lnvaluemale_jspec = asinh(valuemale_jspec)
	bys case_id HHID: gen lnvaluefemale_ospec = asinh(valuefemale_ospec)
	bys case_id HHID: gen lnvaluemale_ospec = asinh(valuemale_ospec)
	
* sales values by manager, winsorized 
	bys case_id HHID: gen lnvaluejoint_jspecw = asinh(valuejoint_jspecw)
	bys case_id HHID: gen lnvaluefemale_jspecw = asinh(valuefemale_jspecw)
	bys case_id HHID: gen lnvaluemale_jspecw = asinh(valuemale_jspecw)
	bys case_id HHID: gen lnvaluefemale_ospecw = asinh(valuefemale_ospecw)
	bys case_id HHID: gen lnvaluemale_ospecw = asinh(valuemale_ospecw)
	
* do not log rainfall 
	
* consumption categories 
	bys case_id HHID: gen lnconsume_agg = asinh(totalexp)
	bys case_id HHID: gen lnconsume_food = asinh(foodexp)
	bys case_id HHID: gen lnconsume_alctob = asinh(alctobexp)
	bys case_id HHID: gen lnconsume_clothfoot = asinh(clothexp)
	bys case_id HHID: gen lnconsume_houseutils = asinh(houseutilsexp)
	bys case_id HHID: gen lnconsume_health = asinh(healthexp)
	bys case_id HHID: gen lnconsume_transpo = asinh(transpoexp)
	bys case_id HHID: gen lnconsume_comm = asinh(commexp)
	bys case_id HHID: gen lnconsume_rec = asinh(recexp)
	bys case_id HHID: gen lnconsume_educ = asinh(eduexp)
	bys case_id HHID: gen lnconsume_hotres = asinh(hotelrestexp)
	bys case_id HHID: gen lnconsume_misc = asinh(miscexp)		
